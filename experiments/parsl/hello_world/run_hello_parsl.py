...
local_thread_executor = ThreadPoolExecutor(max_threads=8, label='local_threads')
local_thread_config = Config(
    executors=[local_thread_executor]
)
...
lassen_htex_config = Config(
    executors=[lassen_htex],
    strategy=None,
)

quartz_htex = HighThroughputExecutor(
    label="Quartz_HTEX",
...
quartz_htex_config = Config(
    executors=[quartz_htex],
    strategy=None
)

all_sites_config = Config(
    executors=[local_htex, quartz_htex, lassen_htex],
    strategy=None
)
...
#parsl.load(lassen_htex_config)
parsl.load(quartz_htex_config)
#parsl.load(all_sites_config)
